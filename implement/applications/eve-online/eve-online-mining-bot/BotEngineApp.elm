...
inSpaceWithOreHoldSelectedExecute : BotDecisionContext -> SeeUndockingComplete -> EveOnline.ParseUserInterface.InventoryWindow -> DecisionPathNode
inSpaceWithOreHoldSelectedExecute context seeUndockingComplete inventoryWindowWithOreHoldSelected =
                                                    ...
                                                    (case context.readingFromGameClient.targets |> List.head of
                                                        ...
                                                        Just _ ->
                                                            ...
                                                            unlockTargetsNotForMining context
                                                                |> Maybe.withDefault
                                                                    (case (context.readingFromGameClient.targets |> List.filter .isActiveTarget) of
                                                                        Just targetAssigned ->
        ...
        endDecisionPath
            (actWithoutFurtherReadings
                ...
                , EffectOnWindow.effectsForLockTarget
                    { startLocation = { x = overviewEntry.uiNode.totalDisplayRegion.x, y = overviewEntry.uiNode.totalDisplayRegion.y}
                    , mouseButton = MouseButtonLeft
                    }
                )
            )
...
overviewWindowEntryRepresentsNoFill entry =
    entry.uiNode
        |> listDescendantsWithDisplayRegion
        |> List.any (.uiNode >> .pythonObjectTypeName >> String.toLower >> String.contains "fill") |> not
